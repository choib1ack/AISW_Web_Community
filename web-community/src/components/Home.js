...
function MakeHomeNoticeList(){
    ...
    return(
        <>
            {noticeData.map(data => (
                <p onClick={()=>ToLink(`/notice/${data.category.toLowerCase()}/${data.id}`)} key={data.notice_id} className={"clickable"}>
...
function MakeHomeBoardList(){
    ...
    return(
        <>
            {boardData.map(data => (
                <p onClick={()=>ToLink(`/board/${data.category.toLowerCase()}/${data.id}`)} key={data.notice_id} className={"clickable"}>
                    {data.title} <span style={{float:"right"}}>{data.created_at.substring(0,10)}</span>
                </p>
